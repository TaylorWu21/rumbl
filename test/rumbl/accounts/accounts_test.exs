defmodule Rumbl.AccountsTest do
  ...
  describe "authenticate_by_email_and_password/2" do
    ...
    test "returns user with correct password", %{user: {:ok, %User{id: id}}} do
    ...
    test "returns unauthorized error with invalid password" do
      assert {:error, :unauthorized} = Accounts.authenticate_by_email_and_password(@email, "badpass")
    end

    test "returns not found error with no matching user for email" do
      assert {:error, :not_found} = Accounts.authenticate_by_email_and_password("bademail@localhost", @pass)
    end
  end
end
